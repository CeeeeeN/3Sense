...
export default function ManageServices() {
  ...
  // Helper: compute {total, byStatus, latest} from a snapshot
  ...
  useEffect(() => {
    ...
    const unsubLivelihood = onSnapshot(collection(db, "livelihoodRegistrations"), snap => {
      setLivelihoodStats(computeStats(snap, "submittedAt"));
    });
  ...
  const StatBar = ({ stats, colorClass }) => {
    const statusColors = {
      ...
      <div style={{ marginTop: "12px", padding: "10px 12px", background: "#f9fafb", borderRadius: "8px", border: "1px solid #e5e7eb" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "4px", marginBottom: "6px" }}>
          {Object.entries(stats.byStatus).map(([status, count]) => {
            ...
            const c = statusColors[key] || { bg: "#f3f4f6", text: "#4b5563" };
            return (
