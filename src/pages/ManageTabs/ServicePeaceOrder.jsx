    ...
    try {
      ...
      if ((newStatus === "responded" || newStatus === "resolved") && residentID && hhID) {
        await createUserNotification(
          ...
          `Your incident report (${selectedReport.refNum || 'N/A'}) has been marked as ${newStatus}. Assigned: ${assignedTanod}`,
          "general",
              ...
              {reports.map(r => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Visual indicator for assigned tanod in list */}
                    <div style={{ fontSize: "0.75rem", color: "#0056b3", marginBottom: "4px", fontWeight: "500" }}>
                      👤 {r.tanod && r.tanod !== "Unassigned" ? r.tanod : "No Tanod Assigned"}
                    </div>
            ...
            {selectedReport ? (
              <div>
                ...
                <div style={{ borderTop: "1px solid #e5e7eb", paddingTop: "20px" }}>
                  <h3 style={{ margin: "0 0 16px 0", fontSize: "1rem" }}>Tanod Assignment & Response</h3>
                  <div style={{ display: "flex", gap: "12px", alignItems: "flex-end" }}>
                    <div style={{ flex: 1 }}>
                      <label style={{ display: "block", color: "#6b7280", fontSize: "0.85rem", marginBottom: "4px" }}>Assigned Group/Tanod</label>
                      <select
                      ...
                      >
                        {TANOD_LIST.map(t => <option key={t} value={t}>{t}</option>)}
                      </select>
                    </div>
                    <div style={{ flex: 1 }}>
                      <label style={{ display: "block", color: "#6b7280", fontSize: "0.85rem", marginBottom: "4px" }}>Status Action</label>
                  ...
                  {saving && <p style={{ color: "#9ca3af", fontSize: "0.8rem", marginTop: "8px" }}>Saving update…</p>}
                </div>

                <div style={{ borderTop: "1px solid #e5e7eb", paddingTop: "20px", marginTop: "24px" }}>
                  <h3 style={{ margin: "0 0 10px 0", fontSize: "1rem" }}>Blotter Updates</h3>
                  {selectedReport.updates?.length > 0 ? (
                    <ul style={{ margin: 0, padding: "0 0 0 18px", color: "#4b5563", fontSize: "0.85rem" }}>
                      {selectedReport.updates.map((u, i) => <li key={i} style={{ marginBottom: "4px" }}>{u}</li>)}
                    </ul>
                  ) : (
              ...
              <div style={{ height: "100%", display: "flex", alignItems: "center", justifyContent: "center", color: "#9ca3af", flexDirection: "column" }}>
                ...
                <p style={{ marginTop: "16px" }}>Select a report to begin dispatch.</p>
              </div>
            )}
          </div>
        </div>
