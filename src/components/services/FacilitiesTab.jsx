  ...
  return (
    <div className="sv-calendar">
      <div className="sv-cal-nav">
        <button className="sv-cal-arrow" onClick={prevMonth}>
          <ChevronLeftIcon />
        </button>
        <span className="sv-cal-title">
          {MONTHS[viewMonth]} {viewYear}
        </span>
        <button className="sv-cal-arrow" onClick={nextMonth}>
          <ChevronRightIcon />
        </button>
      </div>
      <div className="sv-cal-grid">
        {DAYS.map((d) => (
          <div key={d} className="sv-cal-day-label">
            {d}
          </div>
        ))}
      ...
      <div className="sv-cal-legend">
        <span className="sv-legend-item">
          <span className="sv-legend-dot sv-legend-dot--available" />
          Available
        </span>
        <span className="sv-legend-item">
          <span className="sv-legend-dot sv-legend-dot--pending" />
          Pending
        </span>
        <span className="sv-legend-item">
          <span className="sv-legend-dot sv-legend-dot--reserved" />
          Reserved
        </span>
      </div>
    </div>
...
function ReservationForm({
  ...
  const facilityName =
    facility?.name || facility?.title || "Barangay Multi-Purpose Hall";
  ...
  useEffect(() => {
    if (userData) {
      ...
      setForm((f) => ({
        ...
        contactNumber:
          userData.contactNumber != null ? String(userData.contactNumber) : "",
      }));
    }
  }, [userData]);
  ...
  const validate = () => {
    ...
    extra.forEach((f) => {
      ...
      if (f.type === "checkbox") {
        if (!form[f.id]) e[f.id] = "You must confirm this to proceed.";
      } else {
        if (!form[f.id]?.toString().trim()) e[f.id] = "Required.";
      }
    });
    return e;
  };
  ...
  const handleSubmit = async () => {
    ...
    if (hasOverlap) {
      setErrors({
        timeOverlap:
          "The selected time slot conflicts with an existing reservation. Please select a different time.",
      });
      return;
    }
    ...
    return (
      <div className="sv-success-wrap">
        <div className="sv-success-icon">
          <ServiceCheckCircleIcon />
        </div>
        <h3 className="sv-success-title">Reservation Submitted</h3>
        <p className="sv-success-sub">
          Your reservation request has been submitted and is awaiting barangay
          approval.
        </p>

        <div
          className="dr-ref-box"
          style={{
            margin: "1.5rem 0",
            padding: "1rem",
            background: "#f8fafc",
        ...
        <div className="sv-status-badge">
          <ServiceClockIcon /> Pending Approval
        </div>
        <button
    ...
    <div className="sv-form-wrap">
      ...
      <div className="sv-form-body">
        <div className="sv-fields">
          ...
          <div className="sv-field-row">
            <div className="sv-field">
              ...
              <input
                className="sv-input sv-input--readonly"
                value={form.fullName}
                readOnly
              />
            </div>
          ...
          <div className="sv-field">
            ...
            />
            {errors.email && (
              <span className="sv-error-msg">{errors.email}</span>
            )}
          </div>
          <div
            className="sv-field-section-label"
          ...
          <div className="sv-field">
            <label className="sv-label">
              Purpose of Use <span className="sv-required">*</span>
            </label>
            <input
            ...
            />
            {errors.purpose && (
              <span className="sv-error-msg">{errors.purpose}</span>
            )}
          </div>
          <div className="sv-field-row">
            <div className="sv-field">
              <label className="sv-label">
                Start Time <span className="sv-required">*</span>
              </label>
              <input
              ...
              />
              {errors.startTime && (
                <span className="sv-error-msg">{errors.startTime}</span>
              )}
            </div>
            <div className="sv-field">
              <label className="sv-label">
                End Time <span className="sv-required">*</span>
              </label>
              <input
              ...
              />
              {errors.endTime && (
                <span className="sv-error-msg">{errors.endTime}</span>
              )}
            </div>
          </div>
          <div className="sv-field">
          ...
          <div className="sv-field">
            <label className="sv-label">
              Additional Notes <span className="sv-optional">(Optional)</span>
            </label>
            <textarea
              ...
              {facility.customFields.map((f) => {
                if (f.type === "checkbox") {
                  return (
                    <div className="dr-field dr-field--full" key={f.id}>
                      <label
                        className={`dr-checkbox-label${errors[f.id] ? " dr-checkbox-label--error" : ""}`}
                      >
                        <input
                          type="checkbox"
                          checked={!!form[f.id]}
                        ...
                        <span>
                          {f.label}{" "}
                          {f.required && <span className="sv-required">*</span>}
                        </span>
                      </label>
                      {errors[f.id] && (
                        <span className="sv-error-msg">{errors[f.id]}</span>
                      )}
                    </div>
                  );
                }
                return (
                  <div className="dr-field" key={f.id}>
                    ...
                    <input
                      className={`sv-input${errors[f.id] ? " sv-input--error" : ""}`}
                      type={f.type || "text"}
                      value={form[f.id] || ""}
        ...
        <div className="sv-calendar-col">
          <div className="sv-field-section-label">
            Select Date <span className="sv-required">*</span>
          </div>
          <Calendar
          ...
          {pendingDates.includes(form.date) && !errors.timeOverlap && (
            <div className="sv-cal-warning sv-cal-warning--pending">
              <ServiceInfoIcon /> This date has existing reservations. Make sure
              your time slot doesn't overlap.
            </div>
          )}
        </div>
      </div>
      <div className="sv-form-actions">
        <button className="sv-btn-ghost" onClick={onBack}>
          Cancel
        </button>
...
// ── Facilities Tab ──
export default function FacilitiesTab({ onReserve }) {
      ...
      {facilities.map((f) => (
        <div key={f.id} className="sv-facility-card">
          <div className="sv-facility-card__left">
            <div className="sv-facility-card__icon-wrap">
              <BuildingIcon />
            </div>
            <div>
              ...
              <div className="sv-facility-card__desc">
                {f.fullDescription || f.desc}
              </div>
            </div>
          </div>
          <div className="sv-facility-card__right">
            <span
              className={`sv-avail-badge${f.available ? " sv-avail-badge--yes" : " sv-avail-badge--no"}`}
            >
            ...
            {f.available && (
              <button
                className="sv-btn-primary sv-btn-sm"
                onClick={() => onReserve(f)}
              >
                Reserve <ChevronRightIcon />
              </button>
            )}
          </div>
        </div>
...

{
  reservationFacility && (
    <div
      className="rsv-overlay"
      onClick={(e) =>
        e.target === e.currentTarget && setReservationFacility(null)
      }
    >
      <div className="rsv-modal">
        <button
          className="rsv-modal__close"
          onClick={() => setReservationFacility(null)}
          aria-label="Close"
        >
          <svg
            width="16"
