...
function ProgramModal({ program, onClose, onRegister, isRegistering, alreadyRegistered }) {
  ...
  const btnDisabled = alreadyRegistered || isFull || isRegistering;
  const btnLabel    = isRegistering
    ? "Registering..."
    : alreadyRegistered
    ? "Already Registered"
    : isFull
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="pm-scroll" style={{ flex: 1 }}>
          <div className="pm-header">
            <div className="pm-header__tags">
              <span className="pm-badge" style={{ background: c.badge, color: c.text }}>
                {program.tag}
              </span>
              <span
                className={`sv-program-card__status sv-program-card__status--${
                  program.status === "Open" ? "open" : "ongoing"
                }`}
              >
                {program.status}
              </span>
          ...
          <div className="pm-details-grid">
            {[
              ...
              ...(program.slots !== undefined
                ? [{ icon: "slots", label: "Available Slots", value: `${program.slots} slots` }]
                : []),
            ].map((item) => (
              ...
              <ul className="pm-req-list">
                {program.requirements.map((r, i) => (
                  <li key={i} className="pm-req-item">
                    <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke={c.text} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
        ...
        >
          <button className="sv-btn-ghost" onClick={onClose} disabled={isRegistering}>
            Cancel
          </button>
          <button
            className="sv-btn-primary"
            style={{ background: btnBg, opacity: btnDisabled ? 0.75 : 1, cursor: btnDisabled ? "not-allowed" : "pointer" }}
            disabled={btnDisabled}
            onClick={onRegister}
          >
            {/* Checkmark icon when already registered */}
            {alreadyRegistered && (
              <svg
                width="14"
                height="14"
...
export default function ProgramsTab({ userData }) {
  ...
  // ── 2. Check attendees subcollection for each program ────────────────────────
  // Re-runs whenever programs list changes or userId changes
  useEffect(() => {
    if (!activeUserId || programs.length === 0) return;

    const checkRegistrations = async () => {
      setCheckingRegs(true);
      try {
        // Fire all getDoc calls in parallel for efficiency
        const checks = await Promise.all(
          programs.map((prog) =>
            getDoc(doc(db, "Programs", prog.id, "attendees", activeUserId))
          )
        );

        const newRegisteredIds = new Set();
        checks.forEach((snap, i) => {
          if (snap.exists()) newRegisteredIds.add(programs[i].id);
        });
  ...
  const handleRegister = async (programId) => {
    ...
    try {
      ...
      // ✅ Optimistically mark as registered right away — no need to re-fetch
      setRegisteredIds((prev) => new Set([...prev, programId]));
    ...
    <>
          ...
          return (
            <div key={p.id} className="sv-program-card sv-program-card--teal">
              ...
              <div className="sv-program-card__head">
                <span className="sv-program-card__tag">
                  {p.demographic || "General"}
                </span>
                <span
                  className={`sv-program-card__status sv-program-card__status--${
                    p.status === "Open" ? "open" : "ongoing"
                  }`}
                >
                  {p.status || "Upcoming"}
                </span>
              </div>
              <div className="sv-program-card__title">
                {p.description || p.title}
              </div>
              <div className="sv-program-card__desc">
                {p.fullDescription || p.description}
              </div>

              {/* Already-registered badge on the card itself */}
              {isRegistered && (
                <div
                  style={{
                    display: "inline-flex",
                    alignItems: "center",
                    gap: "5px",
                    background: "rgba(49,125,137,0.10)",
                    color: "#317D89",
                    fontSize: "0.75rem",
                    fontWeight: 700,
                    padding: "4px 10px",
                ...
                >
                  <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#317D89" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
                    <polyline points="20 6 9 17 4 12" />
                  </svg>
                  Registered
                </div>
              ...
              <button
                className="sv-program-card__cta"
                onClick={() => setActiveProgram(p)}
                disabled={checkingRegs}
              >
                {checkingRegs ? "Checking..." : "Learn More"} <ChevronRightIcon />
              </button>
            </div>
          );
        })}
      ...
      {activeProgram && (
        <ProgramModal
          ...
          alreadyRegistered={registeredIds.has(activeProgram.id)}
        />
      )}
    </>
  );
