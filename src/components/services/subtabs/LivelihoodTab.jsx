...
const TAG_MAP = {
  manufacturing: { label: "Manufacturing", color: "#1e8a5e" },
  business:      { label: "Business",      color: "#1a56a0" },
  crafts:        { label: "Crafts",        color: "#703381" },
  agriculture:   { label: "Agriculture",   color: "#ca8a04" },
  technology:    { label: "Technology",    color: "#0369a1" },
    ...
    const unsub = onSnapshot(q, (snap) => {
      const data = snap.docs.map(d => {
        ...
        return {
          ...
          slots:       parseInt(raw.slots || raw.requirements?.slots || "0", 10),
  ...
  const handleNext = async () => {
    ...
    if (step === 3) {
      ...
      try {
        ...
        const timeLabel = selectedProgram
          ? `${formatTime(selectedProgram.startTime)}${selectedProgram.endTime ? ` – ${formatTime(selectedProgram.endTime)}` : ""}`
          : "";
